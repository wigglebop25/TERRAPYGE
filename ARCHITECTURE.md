...
## Overview
TERRAPYGE is a physics-informed Graph Neural Network framework for earthquake-induced landslide susceptibility mapping in Cebu City, Philippines. It replaces grid-based approaches with **Slope Units (SUs)** as graph nodes and encodes **spatial and hydrological edge modalities**: spatial adjacency (undirected) + hydrological flow (directed D8).
...
│  (5 classes)│     │  (SAGE)  │     │  (AUC: 0.949)   │     │  (Dual-Edge)    │
└─────────────┘     └──────────────┘     └──────────────────┘     └─────────────────┘
```
